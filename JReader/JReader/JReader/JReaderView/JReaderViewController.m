...
- (void)viewDidLoad {
    ...
    self.headerView.frame = CGRectMake(self.jReaderFrame.origin.x, self.jReaderFrame.origin.y - 44, self.jReaderFrame.size.width, 44);
    
    [self.view addSubview:self.footerView];
    self.footerView.progressLabel.text = [NSString stringWithFormat:@"%zd / %zd", self.jReaderPageIndex + 1, self.jReaderPageCount];
    self.footerView.frame = CGRectMake(self.jReaderFrame.origin.x, self.jReaderFrame.origin.y + self.jReaderFrame.size.height, self.jReaderFrame.size.width, 44);
}

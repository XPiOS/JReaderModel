...
- (ReaderViewController *)createReaderViewController {
    ...
    // 获取页眉
    if([self.delegate respondsToSelector:@selector(PageGenerationManagerHeader:)]){
        // 添加页眉页脚
        self.headerView = [self.delegate PageGenerationManagerHeader:self];
    ...
    // 获取页脚
    if([self.delegate respondsToSelector:@selector(PageGenerationManagerFooter:)]){
        self.footerView = [self.delegate PageGenerationManagerFooter:self];
...
- (void)ReaderViewControllerAddNotes:(NSMutableDictionary *)notesContentDic {
    if([self.delegate respondsToSelector:@selector(PageGenerationManagerAddNotes:)]) {
        [self.delegate PageGenerationManagerAddNotes:notesContentDic];
    }
}
#pragma mark 删除笔记
- (void)ReaderViewControllerDeleteNotes:(NSMutableDictionary *)notesContentDic {
    if ([self.delegate respondsToSelector:@selector(PageGenerationManagerDeleteNotes:)]) {
        [self.delegate PageGenerationManagerDeleteNotes:notesContentDic];
    }
}
...
- (void)AutomaticReadingViewControllerIsShowMenu:(BOOL)isShowMenu {
    if ([self.delegate respondsToSelector:@selector(PageGenerationManagerAutomaticReadingIsShowMenu:)]) {
        [self.delegate PageGenerationManagerAutomaticReadingIsShowMenu:isShowMenu];
    }
}
#pragma mark 退出自动阅读
